...
Napi::Value BLE(const Napi::CallbackInfo& info) {
	...
	if (hdll == NULL || getPos == NULL)
	{
		...
		getPos = (GetPos) ::GetProcAddress(hdll, "getPos_v2");
		if (getPos == NULL) {
	...
	for (int i = 0; i < biSize; i++)
	{
		...
		bi[i].id = oneObj.Get("id").As<Napi::Number>().Int64Value();
	...
	for (int i = 0; i < siSize; i++)
	{
		oneAry = ary.As<Napi::Array>()[i];
		oneObj = oneAry.As<Napi::Object>();
		si[i].id = oneObj.Get("id").As<Napi::Number>().Int64Value();
